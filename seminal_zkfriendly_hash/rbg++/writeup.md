...
* **Kỹ thuật chính:** Algebraic Attack (Tấn công đại số), LCG (Linear Congruential Generator), RSA Common Modulus, Multivariate Polynomial Resultant, Pseudo-Division (Chia giả), Zero Divisors Filtering.
...
```
- các số được đinh nghĩa trên Zmod(N) 
$$
C_i = A_{2i+1} + A_{2i+2} 
$$
- Let $B = m^{1337}$ and $H = m^{-N}$
$$
=> A_{i+1} = A_i^3 B H^{k_i}
$$
- $k_i = {0,1,2}$

Ta sẽ được số N và 13 giá trị đầu của bộ sinh số 

## 2, Exploit Idea 

Ta sẽ sử dụng 3 số đầu $C_0,C_1,C_2$ để tạo các phương trinh 
...
if f1.degree() < f2.degree():
            ...
            if f1.degree() == 0 or f1.degree() == -1:
                break
```
- với f1 và f2 trong code tương ứng với res1 , res2 , khi kết thúc vòng lặp ta có thể thu được f1 là đa thức $f(H) = 0$ (gọi là $res(H)$) à f2 là đa thức $p_1(H) \cdot B + p_0(H) = 0$.
